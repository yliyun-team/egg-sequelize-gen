...
AutoSequelize.prototype.run = function (callback) {
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function (table, _callback) {
      ...
      text[table] += "  const sequelize = app.sequelize; \n";
      text[table] += "  const DataTypes = app.Sequelize.DataTypes; \n\n";

      text[table] += spaces + "return sequelize.define('" + _tableName(table) + "', {\n";
      ...
      _.each(fields, function (field, i) {
        ...
        text[table] += spaces + spaces + _fieldName(field) + ": {\n";
        ...
        _.each(fieldAttr, function (attr, x) {
          ...
          if (attr === "foreignKey") {
            ...
            if (_.isString(defaultVal)) {
              var type = self.tables[table][field].type.toLowerCase();
...
AutoSequelize.prototype.write = function (attributes, callback) {
  ...
  function createFile(table, _callback) {
    fs.writeFile(path.resolve(path.join(self.options.directory, _tableName(table) + '.js')), attributes[table], _callback);
  }
};
...
function _tableName(table) {
  ...
  if (arr[0] === 't') arr.splice(0,1);
  if (arr.length === 1)
    {return table.substr(0, 1).toUpperCase() + table.substr(1);}
...
function _fieldName(field) {
  ...
  if (arr.length === 1)
    {return field;}
  var newArr = arr.map(function (str, index) {
